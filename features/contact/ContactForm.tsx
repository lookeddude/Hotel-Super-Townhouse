...
export function ContactForm() {
  ...
  const [form, setForm] = useState({
    full_name: '', email: '', phone: '', whatsapp: '', subject: '', message: '',
  });
  ...
  if (submitted) {
    return (
      <div className="bg-white rounded-lg border border-outline-variant p-8 flex flex-col items-center text-center gap-4">
        ...
        <button
          onClick={() => { setSubmitted(false); setForm({ full_name: '', email: '', phone: '', whatsapp: '', subject: '', message: '' }); }}
    ...
    <div className="bg-white rounded-lg border border-outline-variant p-8">
      ...
      <form className="space-y-4" onSubmit={handleSubmit} aria-label="Contact form" noValidate>
        ...
        <div className="space-y-1.5">
          ...
          <input id="full_name" type="text" required placeholder="Your full name"
        ...
        <p className="text-xs text-green-700 bg-green-50 border border-green-100 rounded-lg px-3 py-2 flex items-center gap-1.5">
          ...
          Adding your WhatsApp number helps us reply faster! We usually respond within 1 hour on WhatsApp.
        </p>

        {/* Subject */}
        <div className="space-y-1.5">
          <label htmlFor="subject" className="text-sm font-medium text-on-surface">Subject</label>
          <input id="subject" type="text" placeholder="Booking inquiry, room availability, etc."
        ...
        <button type="submit" disabled={submitting}
          className="w-full py-3 bg-primary text-white font-semibold rounded-lg hover:opacity-90 disabled:opacity-60 transition-all flex items-center justify-center gap-2">
          {submitting ? (
